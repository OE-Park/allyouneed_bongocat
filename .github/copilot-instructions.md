...
- 커밋 메시지는 영어 명령형 제목으로 간결하게 작성한다.
- 생성 바이너리, 인증서, 서명 ID, 프로비저닝 프로필, 비밀 정보는 커밋하지 않는다.
- PR 리뷰에서는 리소스 누수, 위험한 캐스트, 정수 잘림, 확인하지 않은 API 실패, UI 스레드 블로킹, Windows/macOS 이식성 회귀를 우선 확인한다.
